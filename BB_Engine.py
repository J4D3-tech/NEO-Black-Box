...
from BB_model import NeoBlackBox, blackbox_loss
from BB_visualizer import visualize_animated_neos

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
MODELS_DIR = os.path.join(BASE_DIR, 'models')
MODEL_SAVE_PATH = os.path.join(MODELS_DIR, 'best_neo_blackbox_model.pth')
...
def prepare_data():
    ...
    if not os.path.exists(csv_path):
        print(f"Error: File not found {csv_path}. Please place the dataset in the 'data' folder.")
        return None
    else:
...
def evaluate_model(data):
    ...
    with torch.no_grad():
        ...
        # Zmienione wycinanie, bo y_pred_test to teraz po prostu [N, 1]
        y_pred_real_moid = scaler_y.inverse_transform(y_pred_test.cpu().numpy())
        ...
        print(f"\n--- Final metrics (Test Set - Black Box) ---")
...
def predict_single(data, name_query):
    ...
    model = NeoBlackBox(input_size=len(data['features_cols'])).to(data['device'])
    model.load_state_dict(torch.load(MODEL_SAVE_PATH))
    model.eval()

    with torch.no_grad():
        y_pred = model(X_tensor)
        predicted_moid_ld = data['scaler_y'].inverse_transform(y_pred.cpu().numpy())[0, 0]
    
    print(f"Predicted MOID (Black Box): {predicted_moid_ld:.2f} Lunar Distances (LD)")
